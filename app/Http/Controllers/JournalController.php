...
$reconciled_trans = GlJournalEntry::where('gl_account_id', $gl_account_id)->where('reversed', 0)->where('reconciled', 1)
->where('office_id', $office_id)
->whereDate('date','<',$end_date)->orderBy('date', 'asc')->get();
$recon_credit = $reconciled_trans->sum('credit');
$recon_debit = $reconciled_trans->sum('debit');
$reconciled_balance = $recon_debit - $recon_credit;
        ...
        return view('journal.reconciliation',
            compact('start_date',
                'end_date', 'close_balance' ,'current_balance','reconciled_balance','office_id', 'data', 'gl_account_id', 'show'));
    }
