...
//determine monthly payment using amortization
    ...
    public static function gl_open_balance($id)
    {
        $transactions = GlJournalEntry::selectRaw(DB::raw('COALESCE(SUM(op_balance_dr),0) open_dr,COALESCE(SUM(op_balance_cr),0) open_cr'))->where('gl_account_id', $id)->where('reversed', 0)->groupBy('gl_account_id')->first();
        return $transactions;
    }
    ...
    {
        $transactions = GlJournalEntry::selectRaw(DB::raw('COALESCE(SUM(op_balance_dr),0) open_dr,COALESCE(SUM(op_balance_cr),0) open_cr,COALESCE(SUM(credit),0) credit, COALESCE(SUM(debit),0) debit'))->where('gl_account_id', $id)->where('reversed', 0)->groupBy('gl_account_id')->first();
        return $transactions;
    }
    ...
    public static function gl_account_reconciled_balance($id)
    {
        $transactions = GlJournalEntry::selectRaw(DB::raw('COALESCE(SUM(credit),0) credit, COALESCE(SUM(debit),0) debit'))->where('gl_account_id', $id)->where('reversed', 0)->where('reconciled', 1)->groupBy('gl_account_id')->first();
        return $transactions;
    }
