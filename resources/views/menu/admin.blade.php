            ...
            @if(Sentinel::hasAccess('loans'))
              ...
              <i class="fa fa-angle-left pull-right"></i>
                    ...
                    <ul class="treeview-menu">
                        ...
                        @if(Sentinel::hasAccess('loans.my_app_loans'))
                            <li><a href="{{ url('loan/my_app_loans') }}"><i
                                            class="fa fa-circle-o"></i> {{trans_choice('general.my',1)}} to be {{trans_choice('general.disbursed',1)}}
                                    <span class="pull-right-container">
                                    <?php
                                     $loan_officer_id = Sentinel::getUser()->id;
                                    ?>
                                        <span class="label label-info pull-right">{{\App\Models\Loan::where('loan_officer_id',$loan_officer_id)->where('status','approved')->count() }}</span>
                                    </span>
                                </a></li>
                        @endif

 
                        @if(Sentinel::hasAccess('loans.branch_approved'))
                            <li><a href="{{ url('loan/branch_app_loans') }}"><i
                                            class="fa fa-circle-o"></i> {{trans_choice('general.branch',1)}} to be {{trans_choice('general.disbursed',1)}}
                                    <span class="pull-right-container">
                                    <?php
                                     $office_id = Sentinel::getUser()->office_id;
                                    ?>
                                        <span class="label label-info pull-right">{{\App\Models\Loan::where('office_id',$office_id)->where('status','approved')->count() }}</span>
                                    </span>
                                </a></li>
                        @endif
              ...
              <i class="fa fa-angle-left pull-right"></i>
                    ...
                    <ul class="treeview-menu">
                        ...
                        @if(Sentinel::hasAccess('accounting.journals.view'))
                            <li class= ""><a href="{{ url('accounting/journal/create_op') }}"><i
                                            class="fa fa-circle-o"></i> Takeon {{trans_choice('general.balance',1)}}
                                </a></li>
                        @endif
                    ...
                    @if(Sentinel::hasAccess('accounting.journals.create'))
                        <li><a href="{{ url('accounting/journal/batch') }}"><i
                                        class="fa fa-circle-o"></i> Batch {{trans_choice('general.journal',1)}} {{trans_choice('general.entry',2)}}
                            </a></li>
                    @endif
